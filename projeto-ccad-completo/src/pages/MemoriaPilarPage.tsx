...
import { ArrowLeft, Star, Building2, UserCog, Layout, Calendar, CalendarPlus, Search, CheckCircle, Circle } from 'lucide-react'
import { BarChart, Bar, XAxis, YAxis, Cell, LabelList, ResponsiveContainer } from 'recharts'
import { supabase } from '@/lib/supabase'
import { useAuth } from '@/contexts/AuthContext'
import { format } from 'date-fns'
import type { Pilar, Fase, Demanda, BenchmarkingRegistro, ConsultoriaMemorial, ProjetoMemorial, ReuniaoAta, Processo, AtividadeFase } from '@/lib/database.types'
...
export function MemoriaPilarPage() {
  ...
  const { data: atividades } = useQuery({
    queryKey: ['atividades-fase-memoria', pilar?.id],
    queryFn: async () => {
  ...
  const concluirAtividade = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['atividades-fase-memoria', pilar?.id] })
      qc.invalidateQueries({ queryKey: ['pilar-memoria'] })
    },
  })
